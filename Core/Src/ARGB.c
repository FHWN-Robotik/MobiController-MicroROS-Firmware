 ...
 */
void ARGB_SetBrightness(uint8_t br) { ARGB_BR = br; }
...
#define ARGB_TIM_CCR CCR4
      ...
      if (HAL_DMA_Start_IT(TIM_HANDLE.hdma[ARGB_TIM_DMA_ID], (uint32_t)PWM_BUF,
                           (uint32_t)&TIM_HANDLE.Instance->ARGB_TIM_CCR, (uint16_t)PWM_BUF_LEN) != HAL_OK) {
        DMA_Send_Stat = HAL_ERROR;
        continue;
      }
 ...
 * @return Scaled value
 */
static inline uint8_t scale8(uint8_t x, uint8_t scale) { return ((uint16_t)x * scale) >> 8; }
